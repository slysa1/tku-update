...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_probe_campaign_model_sources.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Campaign model sources probe script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_CAMPAIGN_MODEL_MOD_NAME' -Value $ModName
    ...
    Write-Host "Launching MW5 Mod Editor campaign/model-source probe..."
