...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_probe_active_campaign_actions.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Active campaign actions probe script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_ACTIVE_ACTIONS_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_ACTIVE_ACTIONS_MAX_ASSETS' -Value ([string]$MaxAssets)
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_ACTIVE_ACTIONS_MAX_DEPTH' -Value ([string]$MaxDepth)
    ...
    Write-Host "Launching MW5 Mod Editor active campaign action probe..."
    Write-Host "  EXE   : $cmdPath"
    Write-Host "  UPROJ : $projectPath"
    Write-Host "  SCRIPT: $scriptPath"
    Write-Host "  MOD   : $ModName"
    Write-Host "  LIMIT : $MaxAssets assets, depth $MaxDepth"
