...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_probe_tku_starmap_bindings.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "StarMap binding probe script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_BINDING_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_BINDING_MAP' -Value $MapAssetPath
