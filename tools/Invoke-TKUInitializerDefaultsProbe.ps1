...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_probe_tku_initializer_defaults.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Initializer defaults probe script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_INITIALIZER_PROBE_MOD_NAME' -Value $ModName
    ...
    Write-Host "Launching MW5 Mod Editor initializer defaults probe..."
