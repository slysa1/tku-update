param(
    ...
    [string]$SourceCluster = '/Game/DLC1/CareerMode/Clusters/Rasalhague_7_10/Rasalhague_7_10_ClusterAsset',
    [int[]]$SystemIds = @(4088, 4089, 4090)
)
...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_patch_tku_active_cluster_diagnostic.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Active cluster diagnostic patch script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_ACTIVE_CLUSTER_DIAG_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_ACTIVE_CLUSTER_DIAG_APPLY' -Value $(if ($Apply) { '1' } else { '0' })
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_ACTIVE_CLUSTER_DIAG_SOURCE' -Value $SourceCluster
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_ACTIVE_CLUSTER_DIAG_IDS' -Value ($SystemIds -join ',')
    ...
    Write-Host "Launching MW5 Mod Editor active-cluster diagnostic patch..."
    Write-Host "  EXE           : $cmdPath"
    Write-Host "  UPROJ         : $projectPath"
    Write-Host "  SCRIPT        : $scriptPath"
    Write-Host "  APPLY         : $Apply"
    Write-Host "  SOURCE CLUSTER: $SourceCluster"
    Write-Host "  SYSTEM IDS    : $($SystemIds -join ',')"
