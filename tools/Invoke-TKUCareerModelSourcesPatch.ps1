...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_patch_tku_career_model_sources.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Career model sources patch script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_CAREER_MODEL_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_CAREER_MODEL_APPLY' -Value $(if ($Apply) { '1' } else { '0' })
    ...
    Write-Host "Launching MW5 Mod Editor career/model-source patch..."
