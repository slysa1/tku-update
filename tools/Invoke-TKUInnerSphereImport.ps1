...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_import_tku_inner_sphere_datatable.py'
...
try {
    ...
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_IMPORT_EXPECT_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_IMPORT_TARGET_ASSET' -Value $TargetAssetPath
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_IMPORT_APPLY' -Value $(if ($Apply) { '1' } else { '0' })
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_ALLOW_ROOT_INNER_SPHERE_OVERRIDE' -Value $(if ($AllowRootOverride) { '1' } else { '0' })
    if ($CsvPath) {
        Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_IMPORT_CSV_PATH' -Value $CsvPath
    }
