...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_patch_tku_game_mode_inner_sphere_class.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Game mode patch script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_GAME_MODE_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_GAME_MODE_APPLY' -Value $(if ($Apply) { '1' } else { '0' })
    ...
    Write-Host "Launching MW5 Mod Editor game-mode InnerSphereClass patch..."
