...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_probe_dlc7_cluster_placement.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "DLC7 cluster placement probe script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_DLC7_CLUSTER_MOD_NAME' -Value $ModName
    ...
    Write-Host "Launching MW5 Mod Editor DLC7 cluster placement probe..."
