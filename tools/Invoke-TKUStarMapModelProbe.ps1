...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_probe_tku_starmap_model.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "StarMap model probe script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_MODEL_MAP' -Value $MapAssetPath
    ...
    Write-Host "Launching MW5 Mod Editor model probe..."
