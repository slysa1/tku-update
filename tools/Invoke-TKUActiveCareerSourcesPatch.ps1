...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_patch_tku_active_career_sources.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Active career sources patch script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_ACTIVE_CAREER_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_ACTIVE_CAREER_APPLY' -Value $(if ($Apply) { '1' } else { '0' })
    ...
    Write-Host "Launching MW5 Mod Editor active-career-source patch..."
