param(
    ...
    [string[]]$AssetPaths = @('/Game/InnerSphereData/StarSystemGenerator'),
    [switch]$Apply
)
...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_save_tku_assets_to_mod.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Save-assets script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_SAVE_ASSETS_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_SAVE_ASSETS_LIST' -Value ($AssetPaths | ConvertTo-Json -Compress)
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_SAVE_ASSETS_APPLY' -Value ($(if ($Apply) { '1' } else { '0' }))
    ...
    Write-Host "Launching MW5 Mod Editor save-assets commandlet..."
    Write-Host "  EXE   : $cmdPath"
    Write-Host "  UPROJ : $projectPath"
    Write-Host "  SCRIPT: $scriptPath"
    Write-Host "  APPLY : $Apply"
    Write-Host "  ASSETS: $($AssetPaths -join ', ')"
