...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_save_tku_starmap_to_mod.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "StarMap save-to-mod script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_EXPECT_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_APPLY' -Value $(if ($Apply) { '1' } else { '0' })
