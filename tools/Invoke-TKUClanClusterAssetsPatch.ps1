...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_patch_tku_clan_cluster_assets.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Clan cluster assets patch script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_CLAN_CLUSTER_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_CLAN_CLUSTER_APPLY' -Value $(if ($Apply) { '1' } else { '0' })
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_CLAN_CLUSTER_PLACEHOLDER_OVERLAYS' -Value $(if ($UsePlaceholderOverlays) { '1' } else { '0' })
    ...
    Write-Host "Launching MW5 Mod Editor TKU clan cluster-assets patch..."
    Write-Host "  EXE                 : $cmdPath"
    Write-Host "  UPROJ               : $projectPath"
    Write-Host "  SCRIPT              : $scriptPath"
    Write-Host "  APPLY               : $Apply"
    Write-Host "  PLACEHOLDER OVERLAYS: $UsePlaceholderOverlays"
