...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_probe_tku_clan_system_coverage.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Clan system coverage probe script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_CLAN_COVERAGE_MAP' -Value $MapAssetPath
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_CLAN_COVERAGE_MOD_NAME' -Value $ModName
    ...
    Write-Host "Launching MW5 Mod Editor clan system coverage probe..."
