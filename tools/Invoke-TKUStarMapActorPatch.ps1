param(
    ...
    [switch]$SpawnProbe,
    [switch]$NoSave,
    [ValidateSet('object', 'class')]
    [string]$SpawnMethod = 'object',
    [int]$Limit = 0,
    [string]$ModName = 'TKUCompatEditorPatch',
    [string]$MapAssetPath = '/Game/Levels/FrontEnd/StarMap'
)
...
$scriptPath = Join-Path $PSScriptRoot 'tku_reference_audit\ue4_patch_tku_starmap_level.py'
...
if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "StarMap patch script not found: $scriptPath"
}

$previousEnv = @{}
try {
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_PROJECT_ROOT' -Value $projectRoot
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_PATCH_MOD_NAME' -Value $ModName
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_PATCH_MAP' -Value $MapAssetPath
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_PATCH_APPLY' -Value $(if ($Apply) { '1' } else { '0' })
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_PATCH_SPAWN_PROBE' -Value $(if ($SpawnProbe) { '1' } else { '0' })
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_PATCH_NO_SAVE' -Value $(if ($NoSave) { '1' } else { '0' })
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_PATCH_SPAWN_METHOD' -Value $SpawnMethod
    Set-TemporaryEnv -Previous $previousEnv -Name 'TKU_STARMAP_PATCH_LIMIT' -Value ([string]$Limit)
    ...
    Write-Host "  PROBE : $SpawnProbe"
    Write-Host "  METHOD: $SpawnMethod"
    Write-Host "  LIMIT : $Limit"
    Write-Host "  NOSAVE: $NoSave"
    Write-Host "  MAP   : $MapAssetPath"
